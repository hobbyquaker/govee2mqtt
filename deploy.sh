...
# govee2mqtt@<name> services there. Same script as in lgtv2mqtt / lgsb2mqtt.
#
# Usage:
...
#
# Optional env vars:
#   REMOTE_DIR   (default: /usr/local/lib/node_modules/govee2mqtt)
#   REMOTE_TMP   (default: /tmp)
#   SERVICE      systemd unit(s) to restart after deploying; default: every active
#                govee2mqtt@<name> instance plus a plain govee2mqtt unit if present
...
#   sudo govee2mqtt --install -n govee -u mqtt://broker
...
if [[ -n "$SERVICE" ]]; then
  ...
  echo "No $PKG service found to restart (install one with: sudo $PKG --install -n govee -u mqtt://broker)"
fi
